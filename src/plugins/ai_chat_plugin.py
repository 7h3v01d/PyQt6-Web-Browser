try:
    from PyQt6.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QTextEdit, QLineEdit, QFileDialog, QMessageBox
...
class AIChatDialog(QDialog):
    def __init__(self, plugin, parent=None):
        ...
        self.setWindowTitle("AI Chat (xAI)")
        ...
        self.api_key = self.plugin.browser.vault.get_api_key("xAI") if self.plugin.browser.vault else None
    ...
    def update_chat_history(self):
        ...
        for entry in self.conversation:
            role = "User" if entry["role"] == "user" else "Grok"
            self.chat_history.append(f"{role}: {entry['content']}\n")

    def send_message(self):
        if not self.api_key:
            QMessageBox.warning(self, "API Key Missing", 
                                "xAI API key not found in the vault. Please add it via the Password Manager in the Settings menu.")
            return
    ...
    def send_api_request(self, query):
        try:
            ...
            payload = {
                ...
                "stream": False,
                "mode": "deep_search" if "search" in query.lower() else "default"
            }
            
            response = requests.post("https://api.x.ai/v1/chat/completions", json=payload, headers=headers, proxies=proxies)
...
class Plugin(Plugin):
    def __init__(self, browser, name="AI Chat Plugin", version="1.0"):
        super().__init__(browser, name, version)

    def add_to_menu(self, menu):
        try:
            action = QAction("AI Chat (xAI)", self.browser)
            action.triggered.connect(self.open_dialog)
            menu.addAction(action)
        except NameError as e:
            self.browser.statusBar.showMessage(f"AI Chat Plugin: QAction not available: {str(e)}", 5000)

    def open_dialog(self):
        if not self.browser.vault:
            QMessageBox.warning(self.browser, "Vault Locked", "You must unlock the vault to use the AI Chat plugin.")
            return
        dialog = AIChatDialog(plugin=self, parent=self.browser)
        dialog.exec()